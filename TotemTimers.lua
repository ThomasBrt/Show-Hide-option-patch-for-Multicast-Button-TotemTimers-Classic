...
function TotemTimers_Slash(msg)
    ...
    if msg ~= "mc on" and msg ~= "mc off" then
        -- Opening TotemTimers options by default
        ...
        -- Commands and function calls to set the Multicast Button Display, while Multicast Button is active - Memø 24012023
        if msg == "mc on" then
            _G["TotemTimers_MulticastDisplayActive"] = true 
            -- Setting the new value : ON, Multicast Button should be visible
            TotemTimers.ActiveProfile.MultiCastDisplay = true           
        else 
            _G["TotemTimers_MulticastDisplayActive"] = false
            -- Storing the new value : OFF, Multicast Button should NOT be visible 
            TotemTimers.ActiveProfile.MultiCastDisplay = false
        end
        TotemTimers.SetMulticastDisplay()
        TotemTimers.ProcessSetting("MultiCastDisplay")
    end
end
